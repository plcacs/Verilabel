...
#-------------the 4 quuestion answer
# ── Step 1: helper to extract all function names from an entire CFG ──────────

def collect_funcs_from_cfg(cfg):
    """
    Walk every node in a CFG, collect all uninterpreted function names
    found inside conj_condition ASTs. Returns a set of function name strings.
    """
    all_funcs = set()
    for n, data in cfg.nodes(data=True):
        cond_ast = data.get('conj_condition')
        if cond_ast is None:
            continue
        calls = _collect_func_calls_in_cond(cond_ast)
        for (callee, arity, args_text) in calls:
            all_funcs.add(callee)
    return all_funcs


# ── Step 2: the main analysis function ───────────────────────────────────────

def analyze_uninterpreted_functions(cfg_before, cfg_after):
    """
    Returns a dict with the result for this ONE pair:
      - has_uf        : bool  (Q1 — either side has at least one UF)
      - same_set      : bool  (Q2 — both sides have identical UF sets)
      - more_in_before: bool  (Q3 — before is strict superset of after)
      - more_in_after : bool  (Q4 — after is strict superset of before)
      - funcs_before  : set   (for logging/debugging)
      - funcs_after   : set
    """
    fb = collect_funcs_from_cfg(cfg_before)
    fa = collect_funcs_from_cfg(cfg_after)

    has_uf         = len(fb) > 0 or len(fa) > 0
    same_set       = fb == fa
    more_in_before = fb > fa   # strict superset: fb ⊃ fa
    more_in_after  = fa > fb   # strict superset: fa ⊃ fb

    return {
        'has_uf':         has_uf,
        'same_set':       same_set,
        'more_in_before': more_in_before,
        'more_in_after':  more_in_after,
        'funcs_before':   fb,
        'funcs_after':    fa,
    }

# ── Step 3: global counters (add near the top with your other globals) ────────

uf_stats = {
    'q1_has_uf':         0,   # pairs where at least one UF exists
    'q2_same_set':       0,   # pairs where both sides have identical UF sets
    'q3_more_in_before': 0,   # before is strict superset
    'q4_more_in_after':  0,   # after is strict superset
    'total_pairs':       0,
}
# ── Per-pair results go to uf_analysis.csv ────────────────────────────────────

def write_uf_row(file_name, count, result):
    output_file = 'uf_analysis.csv'
    ...
    with open(output_file, 'a', newline='', encoding='utf-8') as f:
        ...
        if not file_exists:
            writer.writerow([
                ...
                'q1_has_uf', 'q2_same_set',
                'q3_more_in_before', 'q4_more_in_after'
            ])
        writer.writerow([
            count,
            file_name,
            '|'.join(sorted(result['funcs_before'])),  # e.g. "foo|bar|baz"
            '|'.join(sorted(result['funcs_after'])),
            result['has_uf'],
            result['same_set'],
            result['more_in_before'],
            result['more_in_after'],
        ])


# ── Summary goes to uf_summary.csv ───────────────────────────────────────────

def write_uf_summary():
    """Re-read uf_analysis.csv to compute summary — avoids multiprocessing issue."""
    summary = {
        'total_pairs':       0,
        'q1_has_uf':         0,
        'q2_same_set':       0,
    ...
    if not os.path.isfile('uf_analysis.csv'):
        print("uf_analysis.csv not found — no summary written.")
        return

    with open('uf_analysis.csv', 'r', newline='', encoding='utf-8') as f:
        reader = csv.DictReader(f)
        for row in reader:
            summary['total_pairs']       += 1
            if row['q1_has_uf']          == 'True': summary['q1_has_uf']         += 1
            if row['q2_same_set']        == 'True': summary['q2_same_set']       += 1
            if row['q3_more_in_before']  == 'True': summary['q3_more_in_before'] += 1
            if row['q4_more_in_after']   == 'True': summary['q4_more_in_after']  += 1

    with open('uf_summary.csv', 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
    ...
    print("\n" + "="*50)
    print("UNINTERPRETED FUNCTION ANALYSIS SUMMARY")
    print("="*50)
    for key, val in summary.items():
        print(f"  {key:<25}: {val}")
    print("Saved → uf_summary.csv")
...
def process_file_pair(before_file, after_file, file_name, count):
    try:
        ...
        ##
        result = analyze_uninterpreted_functions(cfg_b, cfg_a)

        uf_stats['total_pairs'] += 1
        if result['has_uf']:         uf_stats['q1_has_uf']         += 1
        if result['same_set']:       uf_stats['q2_same_set']       += 1
        if result['more_in_before']: uf_stats['q3_more_in_before'] += 1
        if result['more_in_after']:  uf_stats['q4_more_in_after']  += 1

        write_uf_row(file_name, count, result)   # ← replaces the old log block
        ##
